...
(function ($) {
    ...
    $.upload = function (options) {
        opts = $.extend(jQuery.uploadDefault, options);
        if (opts.url == '') {
            return;
        }
        ...
        // form中增加数据域
        var formHtml = '<input type="file" name="' + opts.fileName + '" onchange="onChooseFile(this, \'' + opts.exts + '\')">';
...
var onChooseFile = function (fileInputDOM, exts) {
    ...
    //文件后缀
    var selectFileName = $(fileInputDOM).val().toLowerCase();
    var e = selectFileName.substr(selectFileName.lastIndexOf('.') + 1);
    if (exts) {
        if (!e || exts.toLowerCase().indexOf(e) < 0) {
            alert('只能上传后缀名[' + exts + ']的文件！');
            return;
        }
    }
